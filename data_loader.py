...
HISTORICAL_RESULTS_PATH = "C:/Users/jeena/SeleniumSetup/AutomationScript_final/AutomationScript/Results/aggregated_results.json"

LATEST_RESULTS_PATH = "C:/Users/jeena/SeleniumSetup/AutomationScript_final/AutomationScript/Results/OrderPlacementTest_results.json"
...
def load_historical_results():
    ...
    if not os.path.exists(HISTORICAL_RESULTS_PATH):
        print(f" No historical data found at {HISTORICAL_RESULTS_PATH}")
    ...
    try:
        ...
        print(f" Error: Corrupt historical results file at {HISTORICAL_RESULTS_PATH}.")
        return pd.DataFrame(), pd.DataFrame()
...
def load_latest_test():
    ...
    if not os.path.exists(LATEST_RESULTS_PATH):
        print(f" No latest test results found at {LATEST_RESULTS_PATH}")
        return pd.DataFrame(), pd.DataFrame()
    ...
    try:
        ...
        print(f" Error: Corrupt latest test results file at {LATEST_RESULTS_PATH}.")
        return pd.DataFrame(), pd.DataFrame()
...
if __name__ == "__main__":
    ...
    if not latest_summary.empty:
        ...
        print(" No latest test summary available.")

    if not latest_steps.empty:
        print(" Latest test steps loaded successfully.")
        print(latest_steps)
    else:
        print(" No latest test steps available.")
