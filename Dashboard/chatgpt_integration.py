...
# Define paths
ERROR_LOG_PATH = os.getenv("ERROR_LOG_PATH", "Results/OrderPlacementTest_results.json")
CODE_FILE_PATH = os.getenv("CODE_FILE_PATH", "Program.cs")


# OpenAI API Key (Use environment variables for security)
client = openai.OpenAI(api_key="")  # Ensure API key is set in environment variables
...
def analyze_errors_with_chatgpt(errors, code_snippets):
    ...
    1️⃣ **Root Cause Analysis** - Identify the probable reasons behind the errors.
    2️⃣ **Fix Recommendations** - Suggest how to modify the code to prevent these failures.
    3️⃣ **Best Practices** - Any improvements in wait strategies, error handling, and locators.
    4️⃣ **Repeated Issues** - Highlight recurring failures across multiple steps (if applicable).
